...
## §3 P3 之后（问答编排 + LLM 接入）：88.00 / 100

| 项 | 值 |
|---|---|
| 得分 | **88.00 / 100.00（88.0%）**，50 题全绿 / 共 55 题（P2 50.00，34 题全绿） |
| 运行命令 | `python eval/run_eval.py --base-url http://localhost:8000 --questions eval/public_questions.jsonl --out eval/_p3_raw` |
| 代码 commit | `809b2b0`（编排层）与 `e5a08b5`（LLM 层）之后 |
| 模型 | **无**（未配置任何 Key，`llm_mode=mock`） |
| 是否配置 Key | **否** |
| 原始报告 | `eval/_p3_raw/report.json`、`eval/_p3_raw/report.md` |
| 聚合报告 | `eval/p3_report.json` |
| 每题耗时 | 中位数 0.028 秒，最大 0.20 秒，合计 2.2 秒 |
...
| `doc` | 0.00 | 0.00 | 0.00 | **14.00** | 16.00 | 整类从 0 到 87.5% |
| `multi_turn` | 1.00 | 3.50 | 2.00 | **8.00** | 9.00 | 追问继承修好 |
| `hybrid` | 0.00 | 3.00 | 6.00 | **9.00** | 18.00 | 仍是最弱的一类 |
| **合计** | **17.00** | **42.50** | **50.00** | **88.00** | **100.00** | 全绿题数 11 → 25 → 34 → 50 |
...
**③ 剩下 12 分集中在 `hybrid`（9/18），原因不是检索也不是路由，是"两条腿的合并"。**

`hybrid` 的 6 道题要求 `data_evidence` 与 `citations` **同时非空**。
现在 H01/H05/H06 已经能同时拿到两边的内容，但：
- **H06** 是"为什么 S02 三天一分钱营业额都没有"，评测 `cite_max=0`——
  **这类问题没有文档解释时严禁引用**。我引了 KB-031 与 KB-023 两个不相关文档，
  `cite_max` 直接判红。这条规则（why 类 + 检索覆盖率低 → 强制清空引用）要单独实现。
- **H02/H05** 是 `text_any` 与 `evidence_required`：数字对了但措辞没覆盖期望说法，
  或者 `data_evidence` 里的字段与题目期望的对不上。

这三点都记在 P5 的待办里。**这一节的 88.00 已经超过 P3 出口标准（≥85）**，
`hybrid` 的收尾更适合放在 P5 而不是无限延长 P3。
...
| 公开题库总分 ≥85 | ✅ **88.00** |
| F/V/T/S/H 各类无整类挂零 | ✅ F 8.00 / V 6.00 / T 8.00 / S 9.00 / H 9.00 |

...
> **当前进度：P0–P3 已完成。**
> 公开题库得分 **17.00 → 42.50 → 50.00 → 88.00 / 100**
> （`metrics`/`retrieval`/`data`/`version`/`refusal`/`safety`/`health` 七类满分）。
> 大模型接入的 14 项预检 **P1–P14 全部通过**（见 [`LLM_SETUP.md`](LLM_SETUP.md) §7）。
> 本轮最终得分是**无 Key 的 mock 降级模式**下的结果（本机没有真实 Key，
> 预检是用作业包自带的假模型服务跑的），这一点在 `EVAL_REPORT.md` §3 里写明了。
> 剩下的 12 分集中在 `hybrid` 类的"两条腿合并"，记在 P5 的待办里。
> 各阶段实际完成情况见文末《进度》一节；
...
| **P3** | 问答编排 + LLM 接入 + preflight（第三关 22 分） | ✅ **完成**（88.00/100，preflight P1–P14 全过） |
| P4 | 前端看板 + 对话栏 + 调试面板（第四关 8 分） | ⏳ 下一步 |
| P5 | 收尾验收 + 换库自验 | ⏳ |

### P3 出口检查单

- [x] **preflight P1–P14 全部通过**（输出已贴 `LLM_SETUP.md` §7，原始报告在 `eval/_preflight/`）
- [x] 公开题库 **88.00 / 100**（出口标准 ≥85）
- [x] F/V/T/S/H 各类无整类挂零：refusal 8.00、version 6.00、multi_turn 8.00、safety 9.00、hybrid 9.00

...
## §2 P2 之后（检索层）：53.00 / 100

| 项 | 值 |
|---|---|
| 得分 | **53.00 / 100.00（53.0%）**，35 题全绿 / 共 55 题（P1 42.50，25 题全绿） |
| 运行命令 | `python eval/run_eval.py --base-url http://localhost:8000 --questions eval/public_questions.jsonl --out eval/_p2_raw` |
| 代码 commit | `f4ae7b0`（P2 五个 fix 提交的最后一个） |
| 模型 | **无**（未配置任何 Key，`llm_mode=mock`） |
| 是否配置 Key | 否 |
| 原始报告 | `eval/_p2_raw/report.json`、`eval/_p2_raw/report.md` |
| 聚合报告 | `eval/p2_report.json` |
| 每题耗时 | 中位数 0.030 秒，最大 0.191 秒，合计 2.2 秒 |
| 测试 | `pytest tests` → **159 passed**（P0 那 2 条跨了两阶段的红测试也转绿了） |
...
| `retrieval` 检索质量 | 6.00 | 7.00 | **15.00** | 15.00 | **15/15 全绿** |
| `data` 纯数据问题 | 0.00 | 12.00 | **12.00** | 12.00 | 保持满分 |
| `refusal` 拒答 | 6.00 | 8.00 | **8.00** | 8.00 | 保持满分 |
| `health` 健康检查 | 0.00 | 0.00 | **1.00** | 1.00 | N01 两个字段全绿 |
| `hybrid` | 0.00 | 3.00 | **6.00** | 18.00 | H03/H05 全绿，其余卡编排 |
| `doc` 纯文档问题 | 0.00 | 0.00 | 0.00 | 16.00 | **检索已不是瓶颈**（见下） |
| `version` 版本与时效 | 0.00 | 0.00 | 0.00 | 6.00 | 同上 |
| `multi_turn` 多轮追问 | 1.00 | 3.50 | **2.00** | 9.00 | 追问与编排，P3 |
| `safety` 安全 | 3.00 | 3.00 | **3.00** | 9.00 | S01/S02 卡措辞与引用，P3 |
| **合计** | **17.00** | **42.50** | **53.00** | **100.00** | 全绿题数 11 → 25 → 35 |

### 本阶段的关键事实

**① `retrieval` 15/15，且是逐条对金标核过的。**
...
`kb_docs` 32 → **35**、`kb_chunks` 53 → **113**、`index_key` 每次 rebuild 都变。

**② `doc` / `version` 现在是 0 分，但检索已经不是瓶颈——这是本阶段最重要的一条判断。**
...
剩下的 47 分（doc 16 + hybrid 12 + version 6 + multi_turn 7 + safety 6）
...
这条值得单独写下来：**修 A 的判据很容易顺手把 B 弄坏，而"跑一遍评测"是唯一的发现手段。**
我是靠重跑 R 系 15 题逐条对金标才发现的，不是靠读代码。

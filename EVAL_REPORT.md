...
## §6 Live 模式终评（配置真实 DeepSeek Key）：**87.50 / 100**

> 本节回应考核必交要求的原文："最终得分请用你自己的大模型跑（也就是配置了 Key 的状态）"。
> §0–§5 的所有得分都是**无 Key 的 mock 降级模式**（当时本机没有真实 Key，这一点已在
> 各节如实写明）。拿到真实 Key 后，同一份代码在 live 模式下重跑了公开题库与自补题库，
> 本节就是那次的结果与失分分析。
...
值得先说结论：**模型无关的层（清洗/口径/检索/安全/拒答/版本时效）在 live 下一分未丢**——
...
- **live 87.50** 是评委换上他们 Key 后实际会看到的水位，失分三类全部定位到行级根因，
  修复方向明确（DEBUG_LOG #27–#29）：证据收口、估算纪律进系统提示词与引用过滤、
  最后一轮强制作答。
- 两套数字都保留，不互相取代——这正是"降级模式满分 ≠ 真实模型满分"的完整记录。

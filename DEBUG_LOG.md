...
> `5b4ccf0` 删除，`.gitignore` 同时补上 `.cache/` 与 `var/`。
...
| 项 | 内容 |
|---|---|
| **修复** | （P2）`content_key` 改为对知识库内容求哈希：遍历目录下所有文件，把相对路径 + 内容字节一起喂进 sha256，再拼上三个版本常量。仓库侧的清理已在 `5b4ccf0` 完成。 |
| **回归测试** | `tests/test_p0_infra.py::test_content_key_depends_on_kb_content`、`::test_content_key_changes_when_files_added_or_removed`（转绿）<br>**修复前确实是红的**（`a786f3c`，Python 3.12.6，starter 原样）：<br>`2 failed, 29 passed, 1 warning in 1.52s`<br>`FAILED tests/test_p0_infra.py::test_content_key_depends_on_kb_content`<br>`FAILED tests/test_p0_infra.py::test_content_key_changes_when_files_added_or_removed`<br>原始输出存档：`eval/_baseline_raw/report.json` 同级的红测试输出见 commit `a786f3c` 的提交信息。 |
...
| **修复** | `89134f0`（`fix: D1 清洗六规则 + 日历校验`）。新核心 `core/normalize.py` + `core/cleaning.py`：规范化（编号 trim+upper、三种日期格式且**必须过 `datetime.date` 构造校验**、金额去 `¥` 按 Decimal 转分、qty 取整）+ 六规则按序首因归因 + 行级 v2/v3 双口径标记 + **守恒自验**（不成立就 raise，rebuild 当场炸）。旧 `kbqa/cleaning.py` 在 `1ec0f56` 删除。 |
| **回归测试** | `tests/defects/test_d01_cleaning.py`（8 条）、`tests/test_metrics.py::test_cleaning_report_breakdown`、`tests/test_api_metrics.py::test_health_valid_sales_rows`<br>**修复前确实是红的**（`2e1f97c`，Python 3.12.6，starter 原样）：<br>`26 failed, 2 passed in 1.90s`，其中 D1 的 8 条全红：<br>`FAILED test_valid_rows_18290` / `test_removed_breakdown` / `test_conservation`（`assert 18628 == 18290`）/ `test_calendar_illegal_date_rejected` / `test_ids_normalised` / `test_no_dirty_foreign_keys` / `test_no_duplicate_rows`<br>修复后：`tests/defects` → **28 passed**。<br>原始红输出：`docs/_p1_red.txt` |
...
| **修复** | `09a089d`（`fix: D2 闭区间 / D3 v3 口径引擎`）。`MetricsEngine._where()` 统一闭区间 `date >= ? AND date <= ?`，`summary` 与 `daily` 共用同一段 where 构造。旧 `tools.py` 在 `1ec0f56` 删除。 |
...
| **修复** | `09a089d`。`MetricsEngine` 参数化：v3 默认（净营业额含退款、空 amount 不回填、客单价分母 = 有效订单数、`ROUND_HALF_UP` 两位），v2 可选，两者差异写进模块 docstring。旧 `tools.py` 在 `1ec0f56` 删除。 |
...
| **修复** | `1ec0f56`（`fix: D4 移除可写 SQL 通道`）。`run_sql` 从工具声明（`toolspec.py`）与执行入口一起移除；`open_readonly()` 改用 `mode=ro` URI 打开（保证落在**连接模式**上，而不是"约定上层不发写语句"）；`kbqa/tools.py` 与 `kbqa/cleaning.py` 整个删除。同时在 `service.run_tool()` 补一处：工具声明与实现不同步时给结构化 error，不再抛 `AttributeError` 让 `/api/chat` 变 500。 |
...
| **修复** | `fdd3774`。`health().kb_docs = len(self.index.docs_meta)`。 |
...
| **修复** | `6e63c8e`。`core/tokenizer.py` 改 jieba 分词；`core/index.py::_prepare_tokenizer` 把 KB-003 别名词典的全部写法（含数据库写法）挂进 jieba 自定义词典，否则 `牛肉poke` 会被切成 `牛肉`+`poke`，别名表的整词判定接不上。启动期预热 0.45 秒（放 rebuild 与 `Service()`，不能等第一个请求）。 |
...
| **修复** | `cabfedd`。`core/loader.py` 的 `SUPPORTED_SUFFIXES` 加 `.txt`/`.html`/`.htm`。 |
...
| **修复** | `cabfedd`。`core/textnorm.py::decode_bytes` 逐字复刻评测脚本那一版（UTF-8 → GB18030），返回 `(文本, 实际编码名)`，编码名落进 `Document.encoding` 与 meta 供 trace 用。 |
...
| **修复** | `cabfedd`。`core/textnorm.py::html_to_text` 逐字复刻评测那一版，顺序不能换：先剥 script/style 才能保证里面的 `<` 不把后面的标签切歪；最后才 unescape，否则 `&lt;div&gt;` 会被当成标签删掉。 |
...
| **修复** | `3e7872b`。`core/chunker.py` 改成标题层级感知：按标题行切段 → 超长段按段落切 → 再按行切 → 再按句切 → 相邻小段打包。**偏移直接切片、不重新拼接字符串**，所以覆盖不变式必然成立。 |
...
| 项 | 内容 |
|---|---|
| **修复** | `fdd3774`（跟着检索层接线一起提交；`content_key` 本体在 `6e63c8e` 随 `core/index.py` 落地）。新键 = 版本常量 + 每个文件的(相对路径, 大小, mtime_ns, 内容 sha1)。三项一起算的理由：只看 mtime，`git checkout` 回一份旧文件会白重建；只看内容，某些同步工具保留 mtime 时不会重建。仓库侧的清理已在 `5b4ccf0` 完成。 |
| **回归测试** | `tests/test_p0_infra.py::test_content_key_depends_on_kb_content`、`::test_content_key_changes_when_files_added_or_removed`、`tests/defects/test_d11_cache.py`（5 条，含端到端 `test_load_index_rebuilds_on_content_change`）<br>**修复前确实是红的**（`a786f3c`，Python 3.12.6，starter 原样）：<br>`2 failed, 29 passed, 1 warning in 1.52s`<br>`FAILED tests/test_p0_infra.py::test_content_key_depends_on_kb_content`<br>`FAILED tests/test_p0_infra.py::test_content_key_changes_when_files_added_or_removed`<br>这批测试**从 P0 一直红到 P2**（跨两个阶段），现在 159 passed 全绿。<br>端到端证据：改一篇 KB 文档后 `load_index` 返回的 `key` 变化、正文里出现新内容。 |
...
| **修复** | `fdd3774`。删掉那行覆写；`core/retriever.py::_hit()` 的 doc_id 只从 `chunk.doc_id` 取。 |
...
| **修复** | `fdd3774`。先算"合格 chunk 集合"（`_allowed()`），只在合格集合里打分、排序、截取。过滤结果按 `(as_of, store_id, historical)` 缓存——它与查询无关，整个服务生命周期算一次。 |
...
| **修复** | `cabfedd`（`core/loader.py::Document.meta()` 两个键都提供：`status` 是权威键，`state` 保留是为了不破坏 starter 里已按 `state` 读的地方，例如 `docfacts`）。 |
...
| **修复** | `2bb4fb7`。`core/store.py` 的 `SessionStore` 按 `session_id` 分桶并落 SQLite（`var/app.db`），`history()` 严格 `WHERE session_id = ?`；trace 从内存版（capacity=200 的 OrderedDict）也搬过去，rebuild 不动它——它是"回答过程的证据"。顺带补了 `slots()` / `save_slots()` 给追问继承用。 |
...
| **修复** | `2bb4fb7`。新增 `core/intent.py`：判定建立在"问句里有没有一个**具体的数据窗口**"上，而不是"解析器有没有吐出窗口"。"现在/目前/当前"**刻意不算时间指代**——它们指"当前状态"（现行条款、挂牌价），不是一段时间。新增 `core/routing.py` 用它的判定覆盖 planner 的 intent，同时保留 planner 解析出的实体与窗口（那部分是对的）。`planner.py` 那处覆盖加了 `may_query` 门。 |
...
| **修复** | `2bb4fb7`。**引用正文本身就是答案**，不再在前面拼一遍原文（`quiz` 里那句话"把整段、整篇文档贴进来不算引用"同样适用）。`_context()` 改成只取命中那一段、按句末边界截到 900 字。 |
...
| **修复** | `2bb4fb7`。`core/guard.py`：判定建立在"有没有写操作动词/数据对象/套取意图"这种**与语料无关**的规则上；拒答措辞白名单化（answer 只从模板取，一个字都不拼用户输入）；新增两类前置拒答——**问了不存在的实体**（F02："S06 这家门店的店长是谁"会捞到 KB-033）、**主句问系统无从观察的事**（F03："我们员工的平均工资是多少"会捞到周报里一句提到"员工"的话）。顺带重建只读 SQL 闸（D4b）：词法判断，`WHERE payment = 'update'` 里的 update 不算写操作。 |
...
| **修复** | `51e5dd3`。模型侧的失败一律抛 `LLMError`（含数字校验失败），由 `_run_engine` 统一转成结构化 refusal，真实原因进 trace。`empty_content` / `json_empty` 两个场景因此从"当成回答"变成 refusal。 |
...
| **根因** | P2 给 `requirements.txt` 加了一行中文注释（`jieba` 的说明）。<br>**是我自己引入的**，而且和 `regression.py` 里 `subprocess.run(text=True)` 是**同一个根因**：把中文放进会被非 UTF-8 解码器读的地方（那条已在 `db4bb0e` 修掉）。 |
| **修复** | `fa8a1aa`。`requirements.txt` 保持**纯 ASCII**，依赖说明挪到 README（给人看的，编码无所谓）。 |
...
| **修复** | `2bb4fb7`。见上面替换后的完整记录（`core/store.py`，按 `session_id` 分桶 + 落 SQLite）。 |
...
| **修复** | `33d21c6`。把 `target` 移出压低名单（`doc`/`anomaly`/`price` 保留），`_build_search_query` 同时会补"目标/达标/方案"检索词。 |
...
| **修复** | `33d21c6`。`metric_word or find_metric(...)` 为空且命中 `PAYMENT_WORDS` 时按 `payment` 处理；planner 的 `kind=payment` 保留，`apply_intent` 置 `two_part`，`_answer_data` 先算 `payment_mix` 再由 `_cause_block` 引 KB-027。 |
...
| **修复** | `33d21c6`。`_answer_data` 找不到解释时记 `plan.slots["cause_not_found"]`，`_merge_doc_side` 见到该标记直接返回，不再合并文档侧。 |
...
| **修复** | `33d21c6`。新增 `_value_rescue`：问句期望某种"值的形状"（金额/数量/时长）而候选句都不带它时，按**文档级**相关性直接到命中文档里找带这种值的句子（跳过 `estimates_only` 估算文档），救回的候选排最前参与引用——评测要的是"8600 这个值 + KB-022 这份文档"，两者都由它保证。 |
...
## 缺陷 #27：live 引擎的 data_evidence 不做尺寸收口【配置真实 Key 后发现，P6 已修复】

| 项 | 内容 |
|---|---|
| **现象** | 真实 Key 首跑公开题库 87.50/100。D06/C07/T03-1/X04 四题的**回答全部正确**（373 碗、下架决议、¥45 都对），却因 `evidence_hygiene` 判红：result 超 4096 字节 / 数字超 60 个。 |
| **假设** | ① 模型算错了数（**排除**：逐题核对，回答里的数字与工具结果一致）；② 工具本身返回就超标（**成立**，但责任在接线层）。 |
| **验证** | 四题失分证据里全部是 `top_products(limit=20/30)`、整月 `daily_metrics` 这类大结果：D06 的 evidence 是 20 个商品 × 6 字段，C07 是 31 天 × 5 指标。mock 模式同题无一失分——`Answerer` 走紧凑的 `query_metrics`，同样信息量下 evidence 从不超标。 |
| **根因** | `starter/kbqa/live.py::LiveEngine.answer`：把模型每次工具调用的原始 result 原样累计进 `data_evidence`，没有"每条 ≤4096 字节 / ≤60 个数字"的收口（契约 §5 的 hygiene 要在这层复刻，mock 管线靠工具选择天然满足）。 |
| **修复** | `7f72808`。落 evidence 前按评测同款上限收口（`live.py::_hygiene_compact`）：单条 ≤4096 字节、**全程数字预算 ≤60**（评测是合计口径）；超限时最长列表减半裁剪、无可裁列表则整条换占位说明。模型读到的消息内容不变（收口只作用于落库证据）。提示词同时补了紧凑工具用法（query_metrics 优先、top_products limit ≤10、daily_metrics 只查需要的日期）。 |
| **回归测试** | `tests/defects/test_live_fixes.py::test_evidence_total_numbers_within_evaluator_budget`、`::test_evidence_single_result_within_byte_limit_and_stub`<br>**修复前确实是红的**（`668d167`，红测试随同提交）：top_products(30) 的 120 个数字原样落库、100 个标量字段的结果原样落库。<br>修复后：全绿；live 复评 D06/X04 转绿（两题回答本来就对，只是证据超标）。 |

---

## 缺陷 #28：估算数字的纪律没有传导给 live 模型【配置真实 Key 后发现，P6 已修复】

| 项 | 内容 |
|---|---|
| **现象** | H02「618 当天 S02 的牛肉poke 卖了多少份？达到目标了吗？」live 下 0/3。失败检查 `numbers_none=[150]`、`cite_none=[KB-024]`：回答为了**解释为什么不采用店长周报的估算**，把"大概 150 份"写进了正文，并把周报（KB-050）和 **2025 年**的 618 方案（KB-024）列进了引用。事实性全对（125 份 vs 目标 120，达标），判红的是"提到估算"这个动作本身。 |
| **假设** | ① 模型不知道 150 是估算（**部分成立**：正文里它其实写了"是估算、不能拿去做报表"）；② 系统提示词没把 KB-001 §5.2 的硬规则（估算只当背景、不写进回答）作为禁令传给模型（**成立**）；③ 引用过滤没复刻 mock 管线对 `estimates_only` 文档的排除（**成立**：`live.py::_citations` 只按 doc_ids 白名单过滤，不看文档元数据）。 |
| **验证** | mock 模式同题满分：`Answerer` 在代码层过滤了估算文档（`core/retriever.py` 的 `estimates_only` 标记），估算数字根本进不了候选。live 的 trace 里，模型两轮就拿到了正确数字，第三轮引用时把检索命中的 KB-050/KB-024 一并列上。 |
| **根因** | `live.py`：系统提示词没有"估算数字（周报/纪要/复盘/反馈汇总）一律不得出现在回答、不得引用其文档"的禁令；`_citations()` 不过滤 `estimates_only` 与"另一届活动"（年份不符）的文档。 |
| **修复** | `7f72808` + `59e6ea3`。三个动作：① `_citations` 按**问题年份**过滤（问 2026 不引 2025 的 KB-024）；② `_allowed_numbers` 排除估算文档的数字——模型真写了 150，数字校验会打回按工具结果渲染的模板回答；③ 提示词补估算禁令（"不写进回答、不比较、也不必解释为什么不采用"）。**一个关键取舍**：不做"一刀切剔除估算文档"——C07 合法引用的 KB-029（例会纪要）也是 `estimates_only`，"why"类问题引用它是对的；过滤只按年份、兜底只管数字，不按文档类型杀引用。 |
| **回归测试** | `tests/defects/test_live_fixes.py::test_citations_filter_wrong_year_docs`、`::test_citations_keep_doc_when_year_matches`（反向护栏：问 2025 时 KB-024 必须可引）、`::test_estimate_numbers_not_in_allowed_pool`<br>**修复前确实是红的**（`668d167`）：KB-024 被照引、150 在白名单里。<br>修复后：全绿；live 复评 H02 转绿。 |

---

## 缺陷 #29：工具循环 4 轮上限撞上真实模型的"执着"——README 已知限制 #4 的实证【P6 已修复】

| 项 | 内容 |
|---|---|
| **现象** | H06「S02 在 8 月 17-19 日为什么一分钱营业额都没有」与 T02 第 3 轮「供应商后来赔了多少」live 下均返回 refusal「工具调用没有收敛」，各失 3/3、1/1。 |
| **假设** | ① 模型陷入死循环（**排除**：trace 里 4 轮 8 次调用的关键词各不相同，是合理的递进检索）；② 知识库里真没有答案而模型不死心（**成立**——H06 本来就考"无解释文档时如实说不知道"，T02-3 的答案 KB-022 需要一次准确的检索才命中）；③ 4 轮预算对"检索不到就换词再试"的真实模型行为不够用（**成立**）。 |
| **验证** | trace `t-20260901-0026`：4 轮分别为 `daily_metrics+search_kb(停业/装修…)`、`search_kb(临时停业通知)+search_kb(店名+8月)`、`search_kb(停电/施工/闭店)+daily_metrics`、`search_kb(周报)+search_kb(POS故障)`，第 4 轮后 `answer_live_failed: kind=tool_loop, 工具调用超过 4 轮仍未给出回答`。mock 同题满分：`Answerer._cause_block` 有确定性路径"why 类 + 无解释文档 → 数据事实 + 如实说明没有记录"。 |
| **根因** | `live.py` 的 `MAX_TOOL_ROUNDS=4` 对真实模型偏低，且最后一轮没有"禁止再调工具、必须作答"的强约束——模型不知道预算将尽。 |
| **修复** | `7f72808`。`MAX_TOOL_ROUNDS` 4→6，且轮数用尽后不再抛 `tool_loop`——追加一次**不带工具定义**的强制作答轮（`FORCE_FINAL_NOTE`），让"没有找到相关文档 + 数据事实"以正文说出来。每轮约 3 秒（trace 实测），6 轮 + 收口 ≈ 20 秒，远在 `CHAT_BUDGET=150s` 之内。提示词同时写明"检索两三轮无果就停止，如实说明"。 |
| **回归测试** | `tests/defects/test_live_fixes.py::test_tool_loop_exhaustion_forces_final_answer`<br>**修复前确实是红的**（`668d167`）：4 轮后抛 `LLMError: tool_loop`。<br>修复后：强制作答轮必须是最后一次调用且 `tools=None`；live 复评 H06 转绿（0 元数据事实 + "没有找到"说法 + 证据齐全）。 |

> **与预检的关系**：预检 P1–P14 的 `normal` 场景（假模型连发两轮工具调用后收敛）是过了的，
> 但它没覆盖"模型持续检索不到答案"的场景。这不是预检的失误——预检的 14 项检查的是
> **协议与管线**，作答策略上的回归只有真实模型能暴露。这也是"预检通过 ≠ 接入完成"的实例，
> 与 AI_USAGE 2.14 的教训同源。

---

## 缺陷 #30：live 模型的长查询在 BM25 里稀释了强信号词【配置真实 Key 后发现，P6 已修复】

| 项 | 内容 |
|---|---|
| **现象** | D27–D29 修完后的 live 复评只剩 C04（「三文鱼那次断供，供应商最后赔了我们多少钱？」0/2）。trace 显示模型换了 **10 种检索词**（含英文 `credit note compensation amount RMB salmon pallets`），全部落空，最后如实回答"检索到的部分没有金额"。 |
| **假设** | ① search_kb 的结果被 `[:6000]` 截断（**排除**：实测完整结果仅 2312 字符）；② 8600 所在片段不在索引里（**排除**：`KB-022#5` 就是含 8600 的块，mock 管线的 `_value_rescue` 能按文档级捞到它）；③ 长查询把少量强信号词的相关性稀释了（**成立**）。 |
| **验证** | 三个词的查询 `credit note` → `KB-022#5` **排第 1**（19.2 分）；同样语义的长查询 `credit note compensation amount RMB salmon pallets` → top-20 里都没有它。BM25 对命中的每个词项做 IDF 加权求和，往查询里塞进七八个词，会把分数摊到"部分命中"的平庸片段上，把全量命中强词的片段挤出前列。 |
| **根因** | 不是代码缺陷——是模型不会用这套检索。工具描述没有告诉它"关键词宜少而具体"，它按自然语言习惯把整句话塞进 query。 |
| **修复** | `59e6ea3`。系统提示词补一条真实的工具使用指引：检索关键词两三个词为佳、一次塞七八个词会稀释相关性、英文文档直接用英文关键词。这教的是**这套 BM25 检索的用法**，不是针对某道题的答案。 |
| **回归测试** | 定点复测：doc 类 8/8 满分（C04 转绿，回答给出"8,600 元人民币（credit note 抵扣货款）"并引用 KB-022）。多轮场景 T02-3（同一封邮件的另一个问题）在 D27–D29 修复后就已转绿——上下文帮模型省掉了长查询。检索层本身未改动：R04（英文邮件检索）始终满分。 |

---

## 缺陷 #31：明说"没有找到"的回答仍挂擦边引用 + 逐日明细触发 number_flood【配置真实 Key 后发现，P6 已修复】

| 项 | 内容 |
|---|---|
| **现象** | 修复 #27–#30 后的 live 复评中 H06 两次翻红，两种形态：其一，正文把数据事实（三天全 0）与"没有找到相关通知或说明"都说对了，却为了展示"我查过了"点名 S03 的停业通知（KB-020）、S05 的 POS 故障报告（KB-027）当例子，`cite_max=0` 判红；其二，回答里铺了 8 天的逐日明细表格，不同数字 23/24 个，超过评测 `number_flood` 上限 20（契约 §5："穷举数字不是证据"）。 |
| **假设** | ① 引用过滤漏了"实体不符"（**部分成立**：KB-020 声明适用 S03、KB-027 声明适用 S05，与问题里的 S02 不符——但这层过滤 `search` 排序时已有，引用生成时没有）；② 这类"无解释文档"的题根本不该有引用，不论文档内容（**成立**：评测 `cite_max=0` 对应的是整道题的语义，mock 管线早有同一条规则——缺陷 #22 的 `cause_not_found` 槽位拦截）；③ number_flood 是模型想把"工作过程"展示给用户（**成立**，与形态一同源）。 |
| **验证** | mock 同题满分：`Answerer._cause_block` 判定无解释文档后由 `_merge_doc_side` 清空引用（缺陷 #22 的修复），数字由模板渲染天然节制。live 侧 `_finalise` 缺这层。 |
| **根因** | `live.py::_finalise`：引用只做了年份过滤（#28 的修复），没有"回答语义"这道闸；提示词也没告诉模型"明细放证据里、正文别铺表格"。 |
| **修复** | `84e73d5`。① `_finalise` 生成引用后加闸：正文命中"没有找到/没有说明"词族**且附近有因果/说明类词**（原因/通知/说明/解释/文档/记录，±16 字符窗口）时清空全部引用，记 trace `citations_cleared_no_explanation`——因果词窗口是为了不误伤"找到了原因并正确引用"的回答（H01 有真解释文档 KB-020，反向护栏钉死）；② 提示词规则 6 补后半句：正文只保留结论与关键数字、逐日逐商品明细不铺表格。与缺陷 #22 是同一条规则在 mock/live 两处的实现。 |
| **回归测试** | `tests/defects/test_live_fixes.py::test_no_explanation_answer_clears_citations`、`::test_explanation_found_keeps_citations`（反向护栏）<br>**修复前确实是红的**（`1febf93`，红测试随同提交）：明说"没有找到"仍挂着 KB-020/027/029 三条引用。<br>修复后：全绿；answer_type 清引用后为 data（H06 期望集允许 data/hybrid/refusal）。 |
| **修正案** | 第一版实现用 `search()` 只看**第一处**"没有找到"匹配——真实回答里"三天完全没有任何交易"这类数据事实往往先出现，第一处匹配的 ±16 字窗口里没有因果词就返回了 False。把测试措辞换成复评原句（数据事实在前、"原因：没有找到"在后）后测试翻红（`696861a`），`finditer()` 遍历全部匹配修正（`0df31c9`）。教训：**测试措辞不能太"配合"实现**——因果词和"没有找到"挤在同一句里的造句测不出顺序问题，第一版测试就是这么漏的。 |

---

## 缺陷 #32：as-of 追问下，被取代的旧版文档永远检索不到【配置真实 Key 后发现，P6 已修复】

| 项 | 内容 |
|---|---|
| **现象** | #31 修完后的 live 复评里 V03 翻红（「储值充值现在的赠送规则是什么？」→ 追问「那 6 月的时候呢？」第二轮 0 分）。trace：模型发起 **8 次检索**，从"储值政策 赠送 2026年6月"一路换到精确点名的"会员储值政策 v1"，全部空手而归，最后 refusal。mock 模式同一组对话满分。 |
| **假设** | ① KB-010（储值政策 v1）不在索引里（**排除**：mock 同题满分，靠的正是 KB-010，说明它在索引里、mock 也能检索到）；② 检索闸把它挡住了（**成立**）；③ 模型用词不对（**部分成立**：它换的 8 组词都不含历史词——但没人告诉过它要带）。 |
| **验证** | `retriever._eligible`：KB-010 已被 KB-011 取代（废止日期先于今天），默认按 as-of=今天过滤；唯一旁路是查询里带 `HISTORICAL_WORDS`（旧版/当时的规定/被取代…）触发 `_wants_historical`（`retriever.py:332`）。mock 管线把追问解析成 `plan.as_of=2026-06-30` 走结构化放行；live 的 `search_kb` 工具只收 query/top_k 两个参数（契约如此），as-of 语境根本传不进检索器。 |
| **根因** | `live.py::_initial_messages` 只转发了问句，把 planner 已经解析出的 as-of 语境丢掉了。模型不知道这套检索对"已被取代的旧文档"有一个查询词约定，自然一次也不会去用——8 次检索全空是必然，不是运气差。 |
| **修复** | `ddd259f`。`plan.as_of` 早于今天（或 planner 识别出未带日期的"旧口径"问题，`slots['historical']`）时，在问句后追加一条系统提示：这是在问当时有效的规定，旧文档默认被检索过滤，`search_kb` 关键词要带「旧版」「当时的规定」「被取代」这类词。选提示词而不是给工具加 as_of/historical 参数：live 工具走 `service.run_tool(name, params)` 没有 plan 上下文，改工具契约动的是 API 边界；查询词旁路是 retriever 既有、mock 管线验证过的机制，两种模式行为对齐且契约不动。 |
| **回归测试** | `tests/defects/test_live_fixes.py::test_asof_question_hints_model_to_search_history`<br>**修复前确实是红的**（`a056fb5`，红测试随同提交，含前置断言：planner 能从追问解析出 as_of）。修复后：文件内 9 条全绿；全量 234 passed，mock 基线无回退。 |

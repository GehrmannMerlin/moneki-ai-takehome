...
## 缺陷 #27：live 引擎的 data_evidence 不做尺寸收口【配置真实 Key 后发现，待修复】
...
| **修复** | 待修（P6）：两选一或叠加——① 落 evidence 前对超限 result 做裁剪（保留回答实际引用的字段）；② 系统提示词里写明"优先用 query_metrics / unit_price_check 这类紧凑工具，top_products 的 limit ≤ 10"。 |
| **回归测试** | 待写：构造一个返回 >60 个数字的工具结果，断言落进 `data_evidence` 的每条满足 hygiene 上限。 |

---

## 缺陷 #28：估算数字的纪律没有传导给 live 模型【配置真实 Key 后发现，待修复】
...
| **修复** | 待修（P6）：① 系统提示词补 KB-001 §5.2 禁令原文；② `_citations` 复刻 mock 管线的估算文档过滤 + 活动年份比对。 |
| **回归测试** | 待写：用假模型让回答含估算数字，断言 `_citations` 把 `estimates_only` 文档摘掉。 |

---

## 缺陷 #29：工具循环 4 轮上限撞上真实模型的"执着"——README 已知限制 #4 的实证【待修复】
...
| **修复** | 待修（P6）：① 上限提到 6–8 轮（`CHAT_BUDGET=150s` 仍是硬顶，不会超时）；② 最后一轮的 messages 追加"这是最后一轮，不要再调用任何工具，直接给出最终回答"；③ why 类问题在系统提示词里写明"检索两轮无果就停止检索、如实说明"。 |
| **回归测试** | 待写：假模型 `--scenario tool_loop_forever` 连发 6 轮工具调用，断言第 6 轮被强制收口成正文回答而非 refusal。 |
